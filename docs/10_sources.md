...
## Сверка для пакета 3D-визуализации (2026-05-02)

### Context7

- `/mrdoob/three.js`
  - использован для сверки подхода к `GLTFLoader`, `OrbitControls`, animation
    loop, resize behavior и lifecycle cleanup browser-side 3D-viewer.
  - подтверждает текущую стратегию: локальный Three.js renderer загружает GLB,
    обновляется через browser animation loop и должен явно освобождать GPU
    resources.
- `/websites/dash_plotly`
  - использован для сверки `clientside_callback`, `dcc.Store`, local
    `assets/` JavaScript и `window.dash_clientside` как корректного способа
    передавать состояние Dash в browser-side visualization layer.

### Официальные веб-источники

- Three.js `GLTFLoader`
  - https://threejs.org/docs/pages/GLTFLoader.html
  - использовано как reference для загрузки `GLB/GLTF` и сохранения scene graph
    модели.
- Three.js `OrbitControls`
  - https://threejs.org/docs/pages/OrbitControls.html
  - использовано как reference для camera orbit/dolly/pan и необходимости
    `controls.update()` при damping/auto-rotate.
- Three.js cleanup manual
  - https://threejs.org/manual/en/cleanup.html
  - использовано как reference для явного освобождения geometries, materials и
    textures при смене/разборе сцены.
- Three.js `WebGLRenderer`
  - https://threejs.org/docs/pages/WebGLRenderer.html
  - использовано как reference для renderer lifecycle, `dispose()` и renderer
    diagnostics.
- Dash clientside callbacks
  - https://dash.plotly.com/clientside-callbacks
  - использовано как reference для локальных JS callbacks, `dcc.Store`,
    `dash_clientside.callback_context` и передачи состояния в 3D viewer.

Эта сверка не добавляет новых зависимостей. Она закрепляет курс на улучшение
существующего Dash + local Three.js renderer, а не на замену frontend-стека.

## Отечественная источниковая база 3D/ОВК (2026-05-02)

Этот раздел является фильтром качества источников для всех фаз
3D-визуализации и для расчётной части. Любое значение, попавшее в код,
интерфейс или документацию проекта, должно опираться на источник из этого
списка или быть явно помечено как "не базовый источник проекта" (учебный
пример, иллюстрация). Подробные правила использования - в
`docs/38_3d_visualization_ru_source_prompt.md`.

### Нормативная база ОВК и климата

- `СП 60.13330.2020` "Отопление, вентиляция и кондиционирование воздуха"
  - актуализированная редакция СНиП 41-01-2003;
  - базовый российский свод правил по проектированию ОВК;
  - официальный реестр Минстроя России: https://www.minstroyrf.gov.ru/docs/
    (раздел "Документы / Своды правил");
  - федеральный реестр документов в строительстве: https://www.faufcc.ru/.
- `СП 131.13330.2025` "Строительная климатология"
  - действующая редакция; ранняя редакция `СП 131.13330.2020` отменена
    09.09.2025;
  - источник климатических параметров наружного воздуха для расчётных
    сценариев (зима, лето, межсезонье);
  - официальный реестр Минстроя России: https://www.minstroyrf.gov.ru/docs/.
- `ГОСТ 30494-2011` "Здания жилые и общественные. Параметры микроклимата в
  помещениях"
  - параметры микроклимата обслуживаемой зоны: температура, относительная
    влажность, скорость воздуха, оптимальные и допустимые значения;
  - официальный портал стандартов Росстандарта: https://protect.gost.ru/.
- `СанПиН 1.2.3685-21` "Гигиенические нормативы и требования к обеспечению
  ...
  - санитарные нормативы для рабочих мест и общественных помещений; для
    рабочих мест учитывать актуальную редакцию;
  - официальный портал правовой информации:
    http://publication.pravo.gov.ru/.

### Климат и метеоданные РФ

- Минстрой России: https://www.minstroyrf.gov.ru/
  - официальный домен для своды правил (СП), приказы Минстроя и реестр
    нормативно-технических документов в строительстве.
- Росстандарт / ФГИС "Аршин" / protect.gost.ru: https://protect.gost.ru/
  - официальный реестр действующих ГОСТ и сводов правил.
- Официальный портал правовой информации:
  http://publication.pravo.gov.ru/
  - первичная публикация нормативных актов РФ, включая постановления и
    санитарные правила.
- Росгидромет: https://www.meteorf.gov.ru/
  - климатические доклады и пояснительные материалы о климате РФ.
- ФГБУ "ГГО им. А.И. Воейкова": https://voeikovmgo.ru/
  - научно-исследовательские публикации по климату РФ, основа для
    `СП 131.13330` и климатических справочников.

### Учебная и справочная база

- Староверов И.Г. (под ред.) "Внутренние санитарно-технические устройства.
  Часть 3. Вентиляция и кондиционирование воздуха" (Справочник
  проектировщика)
  - базовый отечественный справочник по проектированию приточной и
    вытяжной вентиляции, кондиционированию воздуха;
  - использовать для общего курса расчётов и для пояснений в дипломе.
- Богословский В.Н., Кокорин О.Я., Петров Л.В. "Кондиционирование воздуха
  и холодоснабжение"
  - учебник по СКВ и холодоснабжению, активно используется в
    российских вузах.
- Российские вузовские пособия по ОВК (МГСУ, СПбГАСУ, ТГАСУ, ННГАСУ,
  КГАСУ и др.)
  - использовать для расчётных формул, методик и контрольных примеров.

### Отраслевая практика

- Журнал АВОК (Ассоциация инженеров по отоплению, вентиляции,
  кондиционированию воздуха, теплоснабжению и строительной теплофизике)
  - https://www.abok.ru/
  - инженерные статьи, рекомендации АВОК, обзоры практики, репортажи с
    выставок;
  - рекомендации АВОК (например, "Рекомендации АВОК 5.5-2016. Влажность
    воздуха") можно использовать как отраслевой источник.
- НЭБ (Национальная электронная библиотека): https://rusneb.ru/
- РГБ (Российская государственная библиотека): https://www.rsl.ru/
- eLIBRARY: https://www.elibrary.ru/
- КиберЛенинка: https://cyberleninka.ru/
- ИСТИНА МГУ (научные публикации): https://istina.msu.ru/

### Каталоги оборудования (типовые справочные значения)

Каталожные значения российских производителей используются как типовые
справочные данные. Они не подменяют нормативные значения и должны
сопровождаться пометкой "типовые данные производителя".

- ВЕЗА (производитель вентиляционного оборудования): https://www.veza.ru/
- Арктос (вентиляционное оборудование): https://arktoscomfort.ru/
- Ровен (вентиляционное оборудование, воздуховоды):
  https://rovencom.ru/
- VERTRO (приточные установки): https://vertro.ru/
- "Вентмашиностроение / Вентматик" и другие российские производители ПВУ
  - использовать только официальные сайты или печатные каталоги; не
    использовать переупакованные обзоры третьих сторон.

### Правила использования

- Любой числовой параметр в коде, документации или сценарии сопровождается
  ссылкой на источник из этого раздела или на учебный пример.
- Нормативные значения, учебные формулы, климатические данные и каталожные
  значения оборудования разделять явно. Не смешивать.
- Зарубежные источники (ASHRAE, CIBSE, REHVA, DOE, NREL, NIST и т.п.)
  не использовать как первичную инженерную базу диплома. Уже имеющиеся
  ссылки на DOE/NIST в этом файле относятся к ранним фазам разработки и
  оставлены как технический контекст; для финального ВКР ориентироваться
  на отечественные источники из этого раздела.
- Context7 и официальные docs Three.js/Dash используются только для кода
  фронтенд-слоя (canvas sizing, renderer cleanup, OrbitControls,
  clientside callbacks, dcc.Store) и не являются источником инженерных
  норм ОВК.
